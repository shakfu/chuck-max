 ...
 * by such reordering. Additionally, whether degenerate (vertices or texture
 * coordinates) primitives are present or not will not affect the generated
 * results either. Once tangent space calculation is done the vertices of
 * degenerate primitives will simply inherit tangent space from neighboring non
 * degenerate primitives. The analysis behind this implementation can be found
 * in my master's thesis which is available for download -->
 * http://image.diku.dk/projects/media/morten.mikkelsen.08.pdf Note that though
 * the tangent spaces at the vertices are generated in an order-independent way,
 * by this implementation, the interpolated tangent space is still affected by
 * which diagonal is chosen to split each quad. A sensible solution is to have
 * your tools pipeline always split quads by the shortest diagonal. This choice
 * is order-independent and works with mirroring. If these have the same length
 * then compare the diagonals defined by the texture coordinates. XNormal which
 * is a tool for baking normal maps allows you to write your own tangent space
 * plugin and also quad triangulator plugin.
 */
...
tbool genTangSpaceDefault(
  const SMikkTSpaceContext*
    pContext); // Default (recommended) fAngularThreshold is 180 degrees (which
